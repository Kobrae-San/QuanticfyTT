...
fountainAPIRequest()
  ...
  .then((result) => {
    ...
    result.map((data) => {
      ...
      data.commune.includes("PARIS")
        ? (data.commune = data.commune.replace("EME ARRONDISSEMENT", ""))
        : data.commune; // Treating the Paris case to display it the way i want
      ...
      const commune =
        ...
        data.commune.slice(1).toLowerCase().split(" ").join("");
      tdCity.classList.add("city", "column");
      tdCity.innerText =
        data.commune.charAt(0) + data.commune.slice(1).toLowerCase();

      //   liCityName.innerText = commune;

      cityFilters.add(commune);

      const tdStreetNumber = document.createElement("td");
      tdStreetNumber.classList.add("street-number", "column");
      tdStreetNumber.innerText = data.no_voirie_impair
        ? data.no_voirie_impair
        : data.no_voirie_pair
        ? data.no_voirie_pair
        : "";
      const tdStreetName = document.createElement("td");
      tdStreetName.classList.add("street-name", "column");
      tdStreetName.innerText =
        data.voie.charAt(0) + data.voie.slice(1).toLowerCase();
      const tdavailability = document.createElement("td");
      tdavailability.classList.add("availability", "column");
      const availability =
        data.dispo.charAt(0) + data.dispo.slice(1).toLowerCase();
      ...
      data.debut_ind = data.debut_ind
        ? new Date(data.debut_ind).toLocaleString("fr-FR", dateOptions)
        : "";
      ...
      data.fin_ind = data.fin_ind
        ? new Date(data.fin_ind).toLocaleString("fr-Fr", dateOptions)
        : "";
      ...
      tr.append(
        ...
        tdStreetName,
        tdavailability,
        tdUnavailabilityStart,
        tdUnavailabilityEnd,
        tdUnavailabilityReason
      );
    });
    const cityListContainer = document.querySelector(
      ".filter-tab-city .filter-list"
    );
    ...
    cityFilters.forEach((city) => {
      ...
      liCityName.innerText =
        city.substring(0, 5) == "Paris"
          ? `${city.substring(0, 5)} ${city.substring(5, 7)}`
          : city;

      cityListContainer.appendChild(liCityName);
    });
  ...
  trToPutInactive.forEach((elementToPutInactive) => {
    ...
    if (activeFilter) {
      ...
      if (count === 0) {
        ...
        count > 0 &&
        elementToPutInactive.classList.contains("inactive")
      ) {
...
searchBar.addEventListener("input", () => {
  ...
  const tr = document.querySelectorAll(".data-table-row");
  tr.forEach((element) => {
    const elementChildren = Array.from(element.children).filter((_, index) =>
      [0, 1, 2, 3, 7].includes(index)
    ); // Here i create an array from my object to filter only the column that i want
    ...
    elementChildren.forEach((childrenElement) => {
      ...
      if (
        valueToCompare !== "" &&
        inputValue !== "" &&
        valueToCompare.includes(inputValue)
      ) {
        matchFound = true;
      }
    });
