const equipmentPlaceAPIEndpoint =
...
equipmentAPIRequest()
  .then((result) => {
    ...
    const equipmentFilter = new Set();
    result.map((data) => {
      equipmentFilter.add(data.type);
      ...
      tdName.innerText =
        data.nom.charAt(0).toUpperCase() + data.nom.toLowerCase().slice(1);
      const city = `Paris ${
        data.arrondissement.split(0)[1]
          ? data.arrondissement.split(0)[1].trim()
          : data.arrondissement.split(0)[2].trim()
      }`;
      const cityClass = city.split(" ").join("");
      tdCity.innerText = city;
      cityFilter.add(cityClass);
      tdAddress.innerText = data.adresse
        ? data.adresse.charAt(0).toUpperCase() +
          data.adresse.toLowerCase().slice(1)
        : data.adresse;
    ...
    const equipmentListContainer = document.querySelector(
      ".filter-tab-equipment-type .filters-list"
    );

    equipmentFilter.forEach((equipment) => {
      const liEquipmentName = document.createElement("li");
      liEquipmentName.classList.add("filter-item", "filter-place-type");
      liEquipmentName.innerText = equipment;
      liEquipmentName.id = equipment.split(" ").join("");
      equipmentListContainer.appendChild(liEquipmentName);
    });

    document.querySelectorAll(".filter-place-type").forEach((liElement) => {
      liElement.addEventListener("click", (event) => {
        const targetedElement = event.target;
        targetedElement.classList.contains("active-filter")
          ? targetedElement.classList.remove("active-filter")
